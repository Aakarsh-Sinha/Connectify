...
function Profiletile({user}){
    ...
    useEffect(()=>{
        (async ()=>{
            try{ 
                ...
                const response=await axios.get('http://localhost:5000/api/user/isfriend',{
                    headers:{
                ...
                const response1=await axios.get('http://localhost:5000/api/user/issent',{
                    headers:{
                        Authorization:`Bearer ${token}`,
                        userId:userId
                    },
    ...
    return<div className="p-[1vw] flex justify-between w-[60%]">
